...
//    canvas.ring(Point(300, 200), 200, 230, makeColorPixel(32, 128, 64), Canvas::ANTIALIASING);
    ...
    for (int i = 0; i < 60; ++i) {
      Point p1(100, 400);
      Point p2(50 * cos(2*constant::PI/60*i) + 100, 50 * sin(2*constant::PI/60*i) + 400);
    ...
    for (int i = 0; i < 6; ++i) {
      canvas.circle(Point(50 * cos(constant::PI/3*i) + 100, 50 * sin(constant::PI/3*i) + 100), 10, primaryColors[i], Canvas::ANTIALIASING + Canvas::FILL);
    }
